  ...
  return (
    <div className="p-4 sm:p-6 space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-coffee-900">Expenses</h1>
          <p className="text-coffee-600 text-sm mt-0.5">Record and track all school expenses</p>
        </div>
        <Link
          href="/admin/finance"
      ...
      {total > 0 && (
        <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-6 text-xs sm:text-sm">
          <span className="text-coffee-600">
            <span className="font-semibold text-coffee-900">{total}</span> expense{total !== 1 ? 's' : ''}
          </span>
          <span className="text-coffee-600">
            Total:{' '}
      ...
      {/* Expenses table */}
      {expenses.length === 0 ? (
        <div className="text-center py-16 text-coffee-400">No expenses found.</div>
      ) : (
        <>
          <div className="bg-white border border-coffee-200 rounded-xl overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-coffee-100">
                {expenses.map((e) => (
                  <tr key={e.id} className="hover:bg-coffee-50 transition-colors">
                    <td className="px-3 sm:px-4 py-2.5 font-medium text-coffee-900 text-xs sm:text-sm">{e.name}</td>
                    <td className="px-3 sm:px-4 py-2.5 text-coffee-600 text-xs capitalize">{e.category}</td>
                    <td className="px-3 sm:px-4 py-2.5 text-coffee-500 text-xs whitespace-nowrap hidden md:table-cell">
                      {formatDate(e.expenseDate)}
                    </td>
                    <td className="px-3 sm:px-4 py-2.5 font-medium text-red-600 text-xs sm:text-sm">
