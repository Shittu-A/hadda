...
export async function POST(req: NextRequest) {
  ...
  if (byAdmission) {
    return NextResponse.json({ student: computeStudentBalance(byAdmission) })
  }
  ...
  if (byPhone.length === 1) {
    return NextResponse.json({ student: computeStudentBalance(byPhone[0]) })
  }

  return NextResponse.json({ students: byPhone.map((s) => computeStudentBalance(s)) })
}
